...
let GetILAccessOfILPropInfo (ILPropInfo(tinfo, pdef)) =
    let tdef = tinfo.RawMetadata
    let getAccess = pdef.GetMethod |> Option.map(resolveILMethodRef tdef >> fun mr -> mr.Access)
    let setAccess = pdef.SetMethod |> Option.map(resolveILMethodRef tdef >> fun mr -> mr.Access)
    match getAccess,setAccess with
    | Some ILMemberAccess.Public, _
    | _, Some ILMemberAccess.Public
    | None, None -> ILMemberAccess.Public
    | Some x, None
    | None, Some x -> x
    | Some ILMemberAccess.FamilyOrAssembly, _
    | _, Some ILMemberAccess.FamilyOrAssembly
    | Some ILMemberAccess.Family, _
    | _, Some ILMemberAccess.Family -> ILMemberAccess.Family
    | Some ILMemberAccess.Assembly, _
    | _, Some ILMemberAccess.Assembly -> ILMemberAccess.Assembly
    | Some ILMemberAccess.FamilyAndAssembly, _
    | _, Some ILMemberAccess.FamilyAndAssembly -> ILMemberAccess.FamilyAndAssembly
    | Some ILMemberAccess.CompilerControlled, _
    | _, Some ILMemberAccess.CompilerControlled -> ILMemberAccess.CompilerControlled
    | Some ILMemberAccess.Private, _
    | _, Some ILMemberAccess.Private -> ILMemberAccess.Private
